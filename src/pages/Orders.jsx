...
function Orders() {
  ...
  return (
    <div style={{ backgroundColor: '#050505', minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", color: '#fff' }}>
      <Navbar active="Orders" user={true} />
            ...
            {orders.map(order => {
              ...
              return (
                <div key={order.id} style={{ background: '#0a0a0a', border: '1px solid #151515', borderRadius: '14px', overflow: 'hidden' }}>
                  ...
                  {/* Order Header */}
                  <div style={{ padding: '12px 14px', background: '#0d0d0d', borderBottom: '1px solid #111', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                    <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                      <div>
                        <div style={{ fontSize: '10px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Order ID</div>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: '#22c55e' }}>#{order.id.slice(0, 8)}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '10px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Date</div>
                        <div style={{ fontSize: '12px', fontWeight: '600', color: '#ddd' }}>{new Date(order.created_at).toLocaleDateString()}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '10px', color: '#444', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Total</div>
                        <div style={{ fontSize: '13px', fontWeight: '800', color: '#22c55e' }}>₱{order.total?.toLocaleString()}</div>
                      </div>
                    </div>
                    <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: '20px', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>{order.status}</span>
                  </div>
                    ...
                    <div style={{ padding: '14px', borderBottom: '1px solid #111' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', position: 'relative' }}>
                        ...
                        {[
                          ...
                          return (
                            <div key={step.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px', zIndex: 1, flex: 1 }}>
                              <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: isDone ? '#22c55e' : '#111', border: `2px solid ${isDone ? '#22c55e' : '#333'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', color: isDone ? '#000' : '#333' }}>{isDone ? '✓' : ''}</div>
                              <div style={{ fontSize: '9px', color: isDone ? '#22c55e' : '#444', fontWeight: isDone ? '600' : '400', textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{step.label}</div>
                            </div>
                          )
                        })}
                      </div>
                  ...
                  {/* Delivery Info */}
                  {order.notes && (
                    <div style={{ padding: '10px 14px', borderBottom: '1px solid #111', background: '#0d0d0d' }}>
                  ...
                  <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {order.order_items?.map(item => (
                      <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '42px', height: '42px', borderRadius: '8px', background: '#111', overflow: 'hidden', flexShrink: 0 }}>
                          {item.products?.image_url ? <img src={item.products.image_url} alt={item.products?.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>🎸</div>}
                        </div>
                        <div style={{ flex: 1 }}>
